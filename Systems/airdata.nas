...
var update_vars = func( dt ) {
    ...
    setprop("/apm/rightaileron",  round100(getprop("/surface-positions/right-aileron-pos-norm"))); 
    setprop("/apm/leftaileron",  round100(getprop("/surface-positions/left-aileron-pos-rad"))); 


    setprop("/apm/elevator", round100(getprop("/surface-positions/elevator-pos-rad"))); 
    setprop("/apm/rudder",   round100(getprop("/surface-positions/rudder-pos-norm"))); 
    setprop("/apm/throttle", 1-round100(getprop("/controls/engines/engine[1]/throttle")));
    # setprop("/apm/throttle2", round100(getprop("/engines/engine[1]/throttle-cmd-norm")));    

    setprop("/apm/groundspeed", round10(0.5147222*getprop("/instrumentation/gps/indicated-ground-speed-kt")));

    # airspeed-kt is actually in feet per second (FDM NET bug)
    setprop("/apm/airspeed", round10(0.514722*getprop("/velocities/airspeed-kt")));
}
